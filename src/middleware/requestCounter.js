...
import { requestCounterConfig } from "./config.js";

const MAX_TRACKED_PATHS = requestCounterConfig.maxTrackedPaths;
...
export function createRequestCounter() {
  return async (c, next) => {
    ...
    if (
      stats.requestsByPath[path] !== undefined ||
      Object.keys(stats.requestsByPath).length < MAX_TRACKED_PATHS
    ) {
...
export function getRequestStats() {
  ...
  const avgRequestsPerSecond = (stats.totalRequests / (uptime / 1000)).toFixed(
    2
  );
